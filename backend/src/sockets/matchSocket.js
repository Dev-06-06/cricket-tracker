...
async function handleWicketInningsCompletion(match, matchId, io, options = {}) {
  ...
  await match.save();
  const completed = await completeMatchWithStats({ matchId, resultMessage, io });
...
async function finalizeSecondInningsIfMatchEnded(match, matchId, io, totalValidBalls) {
  ...
  await match.save();
  const completed = await completeMatchWithStats({ matchId, resultMessage: evaluation.resultMessage, io });
  return completed ? "match_complete" : "none";
}

// ─── handleMaxOversTransition ─────────────────────────────────────────────────
async function handleMaxOversTransition(match, matchId, io, totalValidBalls, options = {}) {
  const { suppressStateEmit = false } = options;
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("setNewBatter", async ({ matchId, batter }) => {
      try {
        ...
        const batterEntry = match.playerStats.find(
      ...
      in team ${battingTeam} — creating minimal entry`);
          match.playerStats.push({
            ...
            batting: { runs: 0, balls: 0, fours: 0, sixes: 0, dismissalType: "" },
            bowling: { overs: 0, balls: 0, runs: 0, wickets: 0, wides: 0, noBalls: 0 },
          });
        }
        ...
        if (batterEntry && batterEntry.isBenched) {
          ...
          match.current.benchedPlayers = (match.current.benchedPlayers || [])
            .filter(n => n !== batter);
          match.markModified("current");
        }
      ...
      try {
        ...
        const savedMatch = await saveWithRetry(matchId, async (match) => {
          ...
          if (match.status !== "live" && match.status !== "innings") {
            socket.emit("matchError", { message: "Cannot bench: match not in play" });
            return;
          }
          ...
          if (batterName !== striker && batterName !== nonStriker) {
            socket.emit("matchError", { message: "Player is not currently at crease" });
            return;
          }
          ...
          if (available.length === 0) {
            socket.emit("matchError", {
              message: "Cannot bench: no available replacement batters"
            });
            return;
          }
          ...
          if (!ps) {
            socket.emit("matchError", { message: "Player stats not found" });
            return;
          }

          // RISK: already benched guard
          if (ps.isBenched) {
            socket.emit("matchError", { message: "Player is already benched" });
            return;
          }
    ...
    socket.on("benchAndReplace", async ({ matchId, batterName, replacementName }) => {
      try {
        const match = await Match.findById(matchId);
        if (!match) return;
        ...
        const benchedPs = match.playerStats.find(
          p => p.name === batterName && p.team === battingTeam
        ...
        const vacatedPosition = batterName === striker
          ? "striker" : "nonStriker";
        ...
        const replacementPs = match.playerStats.find(
          p => p.name === replacementName && p.team === battingTeam
        ) || match.playerStats.find(
          p => p.name === replacementName
        );
    ...
    socket.on("overBreakCommit", async ({ matchId, payload }) => {
      ...
      try {
        ...
        if (!match.current.overBreakPending) {
          socket.emit("matchError", { message: "No over break is currently pending" });
          return;
        }

        // RISK: innings must still be in a breakable state
        if (match.status === "completed" || match.status === "innings_complete") {
          socket.emit("matchError", { message: "Cannot commit over break: innings over" });
          return;
        }
        ...
        if (!newBowler) {
          socket.emit("matchError", { message: "Bowler selection is required" });
          return;
        }
        // RISK: benched player must not be selected as bowler
        const bowlerPs = match.playerStats.find((p) => p.name === newBowler);
        if (bowlerPs?.isBenched) {
          socket.emit("matchError", { message: "Benched player cannot bowl" });
          return;
        }
        ...
        if (newTotalOvers != null) {
          const completedOvers = Math.floor((match.current.ballsBowled || 0) / 6);
        ...
        if (addPlayers && addPlayers.length > 0) {
          ...
          for (const ap of addPlayers) {
            ...
            const snapshot = { playerId, name: ap.name.trim(), photoUrl: ap.photoUrl || "" };
            ...
            const alreadyExists = match.playerStats.some(
              (p) => p.playerId?.toString() === playerId?.toString() && p.team === team,
            );
            if (!alreadyExists) {
              match.playerStats.push({
                playerId,
        ...
        if (reshuffles && reshuffles.length > 0) {
          for (const r of reshuffles) {
            ...
            const toTeamKey = r.toTeam === match.team1.name
              ? "team1" : "team2";
            ps.team = toTeamKey;
            ...
            const snap = { name: r.playerName, playerId: ps.playerId, photoUrl: "" };
        ...
        if (setJokers && setJokers.length > 0) {
          for (const sj of setJokers) {
            // Find existing entry
            const existing = match.playerStats.find((p) => p.name === sj.playerName);
            ...
            if (!otherExists) {
              ...
              const snap = { name: existing.name, playerId: existing.playerId, photoUrl: "" };
        ...
        if (dissolveJokers && dissolveJokers.length > 0) {
          for (const dj of dissolveJokers) {
            ...
            const permanentTeamKey = dj.permanentTeam === match.team1.name
              ? "team1" : "team2";
            const otherTeamKey = permanentTeamKey === "team1"
              ? "team2" : "team1";
      ...
      try {
        ...
        const savedMatch = await saveWithRetry(matchId, async (match) => {
          ...
          if (isWicket) {
            ...
            wicketTransition = await handleWicketInningsCompletion(match, matchId, io, { suppressStateEmit: true });
          ...
          maxOversTransition = await handleMaxOversTransition(match, matchId, io, totalValidBalls, {
            suppressStateEmit: true,
          });
          if (maxOversTransition !== "none") return;

          secondInningsTransition = await finalizeSecondInningsIfMatchEnded(match, matchId, io, totalValidBalls);
        ...
        const rawExtraType = deliveryPayload.extraType === "noBall" ? "no-ball" : deliveryPayload.extraType || "none";
        const isValidBallOuter = isValidBallType(rawExtraType);
        const justCompletedOver = isValidBallOuter &&
        ...
        if (justCompletedOver &&
          ...
          savedMatch.current.overBreakPending = true;
          savedMatch.markModified("current");
    ...
    socket.on("umpire_update", async ({ matchId, deliveryData }) => {
      try {
        ...
        const justCompletedOverU = isValidBallU &&
          savedMatch.current.ballsBowled % 6 === 0 &&
          savedMatch.current.ballsBowled > 0;
        if (justCompletedOverU &&
            savedMatch.status !== "innings_complete" &&
            savedMatch.status !== "completed") {
          savedMatch.current.overBreakPending = true;
          savedMatch.markModified("current");
